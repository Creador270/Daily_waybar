...
connected=$(printf '%s\n' $(nmcli -t -f DEVICE,TYPE,STATE device) | awk -F: '($2=="wifi" || $2=="ethernet") && $3=="connected" {print $2}')
...
if [ -z "$connected" ]; then
  ...
  # Get the current Wi-Fi information
  wifi_info=$(nmcli -t -f active,ssid,signal,security,device,bssid,chan,freq dev wifi | grep "^yes")
  # Some defaults
  ip_address=$(ip -4 addr show dev $(echo $wifi_info | awk -F: '{print $5}') | awk '/inet / {print $2}')
  # gateway="127.0.0.1"
  # mac_address="N/A"
  ...
  chan=$(echo $wifi_info | awk -F: '{print $12}')
  freq=$(echo $wifi_info | awk -F: '{print $13}')
  chan="$chan ($freq)"
